...
inline void theme_path_install() {
    ...
    ClickButtonMode3(hWnd_Window, L"主题补丁安装工具", L"&Next >");
    ClickButtonMode1(hWnd_Window, L"主题补丁安装工具", L"I &accept the terms of the License Agreement");
    ClickButtonMode3(hWnd_Window, L"主题补丁安装工具", L"&Next >");
    ClickButtonMode3(hWnd_Window, L"主题补丁安装工具", L"I &Agree");
    ClickButtonMode3(hWnd_Window, L"主题补丁安装工具", L"&Install");
    ClickButtonMode3(hWnd_Window, L"主题补丁安装工具", L"Installation Complete");
    ClickButtonMode3(hWnd_Window, L"主题补丁安装工具", L"&Next >");

    INFO_(L"[主题补丁安装工具]查找 稍后重启");
    //稍后重启-查找
    ...
    while (temp == NULL) {
        ...
        temp1 = FindChildControlExact(hWnd_Window, L"Donation via Paypal");
        if (temp1!=NULL) {
            INFO_(L"[主题补丁安装工具]已找到 捐赠按钮");
            INFO_(L"[主题补丁安装工具]主题补丁已安装或不兼容 退出");
            //前置
            SetForegroundWindow(hWnd_Window);
            enter_key();
            return;
